...
pub fn attack(input: Res<Input<KeyCode>>, mut player: Query<&Transform, With<Player>>, mut enemy: Query<&mut Stats, With<Enemy>>, mut commands: Commands){
    let player_transform = player.single_mut();
    
	if input.just_pressed(KeyCode::P) { //If the Player threw a Punch
        commands
		...
		//code to signal a collision
		let mut enemy_stats: Mut<Stats> = enemy.single_mut(); //new variables to access health of enemy
		enemy_stats.health -= 10.0; //change enemy health to absorb punch

	}
    if input.just_pressed(KeyCode::K){ //If the Player threw a Kick
        commands
		.spawn_bundle(SpriteBundle {
			sprite: Sprite {
				color: Color::GREEN,
		...
		let mut enemy_stats: Mut<Stats> = enemy.single_mut(); //new variables to access health of enemy
		enemy_stats.health -= 5.0; //change enemy health to absorb punch
